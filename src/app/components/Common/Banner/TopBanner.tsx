    ...
    return (
        <div className='relative w-full md:h-[480px] max-w-full flex flex-col justify-center items-center'>
            ...
            {BannerContainerDataContent &&
                <div className='z-10 md:absolute left-0 right-0 top-[50%] transform -translate-y-[50%] px-8 py-8 rounded-xl bg-white dark:bg-black w-[95%] md:w-[55%] h-auto mx-auto flex justify-center items-center'>
                    <img
                        className='md:w-[8rem] w-16 md:h-[8rem] h-16 object-contain'
                        src="https://img.icons8.com/?size=256&id=20523&format=png"
                        alt='banner-icon'
                    />
                    <h1 className='md:text-6xl text-3xl font-extrabold text-black dark:text-white'>{BannerContainerDataContent?.Header?.content?.title}</h1>
                </div>
            }
        </div>
    );
